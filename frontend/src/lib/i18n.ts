export type Language = 'en' | 'es' | 'pt' | 'fa' | 'th';
...
const translations: Record<Language, Record<string, string>> = { en, es, pt, fa, th };
...
function formatIcuPlural(template: string, params: Record<string, string | number>, lang: Language): string {
  return template.replace(PLURAL_RE, (_match, name: string, cases: string) => {
    ...
    const locale = lang === 'es' ? 'es-AR' : lang === 'pt' ? 'pt-BR' : lang === 'fa' ? 'fa-IR' : lang === 'th' ? 'th-TH' : 'en';
